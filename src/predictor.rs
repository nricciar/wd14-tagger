...
fn load_wd14_labels(
    ...
    let (mut names, mut ratings, mut general, mut character) =
        (vec![], vec![], vec![], vec![]);
...
fn prepare_wd14(img: &DynamicImage, size: usize) -> Array4<f32> {
    ...
    let resized = image::imageops::resize(
        &canvas, size as u32, size as u32,
    ...
    for y in 0..size {
        for x in 0..size {   // FIX: original erroneously started at 1, skipping column 0
...
fn prepare_dinov3(img: &DynamicImage) -> Array4<f32> {
    ...
    let snap = |x: u32| -> u32 {
        DINO_PATCH_SIZE.max(
            ((x as f32 * scale).round() as u32 / DINO_PATCH_SIZE) * DINO_PATCH_SIZE,
        )
    };
    let new_w = snap(w);
    let new_h = snap(h);
...
fn build_output(
    ...
    OutputData { general, rating, characters: character, embedding }
}
...
impl Predictor {
    ...
    pub fn wd14()   -> Self { Self::new(ModelKind::Wd14)   }
    pub fn dinov3() -> Self { Self::new(ModelKind::DINOv3) }

    // ── Lazy initialisation ───────────────────────────────────────────────────

    fn ensure_loaded(&mut self) {
        if self.model.is_some() { return; }
        match self.kind {
    ...
    pub fn predict(
        ...
        let (scores, embedding): (Vec<f32>, Vec<f32>) =
            match self.model.as_mut().unwrap() {
        ...
        build_output(
            &scores, embedding,
            &self.tag_names, &self.rating_i, &self.general_i, &self.character_i,
            g_th, g_mcut, c_th, c_mcut, exclude,
        )
    }
}